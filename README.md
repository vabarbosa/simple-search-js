...
* __Javascript__
	...
	__serviceUrl__ - (_Required_) the url for the Simple Search Service to connect to  
...
|---|---|---|
| inputField | string or HTML DOM element | _Required_ - The search input field. It may be an `input` HTML DOM element or a CSS selector string for the `input` element. For example, "__#searchInput__" to select an `input` with an ID of _searchInput_ |
